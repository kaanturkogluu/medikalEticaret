                ...
                <div class="overflow-hidden shadow-2xl shadow-slate-200/50 rounded-[32px] border border-white bg-white/50 backdrop-blur-sm">
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50/50">
                            <tr>
                                <th scope="col" class="py-5 pl-8 pr-3 text-left text-xs font-black uppercase tracking-widest text-slate-400 italic">Logo</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-white/80 transition-colors group">
                                <td class="whitespace-nowrap py-5 pl-8 pr-3">
                                    @if($brand->logo)
                                        <div class="h-12 w-12 rounded-xl border border-gray-100 p-1 bg-white shadow-sm flex items-center justify-center overflow-hidden">
                                            <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="max-w-full max-h-full object-contain">
                                        </div>
                                    @else
                                        <div class="h-12 w-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-300 font-black italic text-xs border border-gray-100 uppercase tracking-tighter">
                                            LOGO
                                        </div>
                                    @endif
                                </td>
                                <td class="whitespace-nowrap px-3 py-5">
                                    <div class="text-sm font-black text-slate-900 italic tracking-tighter uppercase">{{ $brand->name }}</div>
                                </td>
                                ...
                                <td class="relative whitespace-nowrap py-5 pl-3 pr-8 text-right text-sm font-medium">
                                    <div class="flex justify-end gap-3 translate-x-4 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                                        ...
                                        <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="POST" onsubmit="return confirm('Bu markayı silmek istediğinize emin misiniz?')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <td colspan="5" class="px-3 py-20 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <div class="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-4 ring-8 ring-slate-50/50">
                                            <i class="fas fa-industry text-4xl text-slate-200"></i>
                                        </div>
                                        <p class="text-lg font-black italic tracking-tighter text-slate-400 uppercase">Herhangi bir marka bulunamadı.</p>
                                        <a href="{{ route('admin.brands.create') }}" class="mt-6 text-sm font-black italic tracking-tighter text-[var(--primary-color)] hover:underline underline-offset-4 uppercase">Hemen İlk Markayı Ekle</a>
                                    </div>
                                </td>
                            </tr>
                            @endforelse

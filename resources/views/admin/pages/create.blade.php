...
@section('title', 'Yeni Metin Ekle')

@section('content')
<div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center mb-8">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-black italic tracking-tighter text-slate-900 uppercase underline decoration-brand-500 decoration-4 underline-offset-8">Yeni Metin Ekle</h1>
            <p class="mt-4 text-sm text-gray-500 font-medium italic">Sistemde bulunmayan yeni bir sözleşme veya politika ekleyin.</p>
        </div>
    ...
    <form action="{{ route('admin.pages.store') }}" method="POST" class="space-y-8 bg-white/50 backdrop-blur-xl p-8 rounded-[40px] border border-white shadow-2xl shadow-slate-200/50 overflow-hidden relative group">
        @csrf
        ...
        <div class="relative space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="title" id="title" required value="{{ old('title') }}" placeholder="Örn: KVKK Aydınlatma Metni"
                ...
                <div class="flex items-center gap-6 p-6 bg-slate-50/50 rounded-3xl border border-slate-100 h-full">
                    <div class="h-10 w-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-brand-500 text-center">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="15" placeholder="Metin içeriğini buraya girin..."
                          class="block w-full px-5 py-4 bg-white border border-slate-200 rounded-3xl text-sm font-medium text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all min-h-[400px]">{{ old('content') }}</textarea>
            ...
            <div class="pt-6 border-t border-slate-100 flex gap-4 text-center">
                <button type="submit" class="flex-grow px-10 py-5 bg-slate-900 text-white font-black italic uppercase tracking-tighter rounded-2xl shadow-2xl shadow-slate-200/50 hover:bg-slate-800 transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center gap-3">
                    <i class="fas fa-plus text-brand-400"></i> Sayfa Oluştur
                </button>

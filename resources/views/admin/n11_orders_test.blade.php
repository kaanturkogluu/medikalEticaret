...
<div class="space-y-6" x-data="{ 
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <div class="h-8 w-8 bg-emerald-600 rounded-lg flex items-center justify-center text-white shadow-lg shadow-emerald-500/20">
                    <i class="fas fa-store text-xs"></i>
                </div>
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight">N11 Sipariş Yönetimi (Ham Veri)</h2>
            </div>
            <p class="text-sm text-slate-500">N11 API'den gelen canlı paket verilerini Trendyol formatında buradan inceleyebilirsiniz.</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <button onclick="window.location.reload()" class="px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 transition-colors flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Toplam Paket</p>
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black text-emerald-600">{{ count($orders->where('shipmentPackageStatus', 'Created')) }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Toplanıyor (Picking)</p>
            <h3 class="text-2xl font-black text-amber-500">{{ count($orders->where('shipmentPackageStatus', 'Picking')) }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Kargolananlar</p>
            <h3 class="text-2xl font-black text-blue-600">{{ count($orders->where('shipmentPackageStatus', 'Shipped')) }}</h3>
        </div>
    </div>
        ...
        <table class="w-full text-left">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Paket ID</th>
                    ...
                    <th class="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Son Güncelleme</th>
                    <th class="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">İşlem</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-50">
                @forelse($orders as $package)
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    <td class="px-8 py-5">
                        <div class="flex flex-col">
                            <span class="text-xs font-black text-slate-800 tracking-tighter">#{{ $package['orderNumber'] ?? 'N/A' }}</span>
                            <span class="text-[10px] font-bold text-slate-400">{{ $package['customerfullName'] ?? 'Müşteri Bilirtilmemiş' }}</span>
                        </div>
                    </td>
                    <td class="px-8 py-5">
                        <span class="text-xs font-bold text-slate-600 tabular-nums">{{ $package['id'] ?? 'N/A' }}</span>
                    </td>
                    <td class="px-8 py-5 text-center">
                        @php $status = $package['shipmentPackageStatus'] ?? 'Unknown'; @endphp
                    ...
                    <td class="px-8 py-5 text-right">
                        <span class="text-sm font-black text-slate-900 tabular-nums">{{ number_format($package['totalAmount'] ?? 0, 2, ',', '.') }} ₺</span>
                    </td>
                    <td class="px-8 py-5">
                        <span class="text-[10px] font-bold text-slate-500">
                            {{ isset($package['lastModifiedDate']) ? date('d.m.Y H:i', $package['lastModifiedDate'] / 1000) : 'N/A' }}
                        </span>
                    </td>
                    <td class="px-8 py-5 text-right">
                        <button @click="selectedOrder = {{ json_encode($package) }}" class="p-2 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-emerald-500 hover:text-emerald-600 transition-all">
        ...
        <div @click.away="selectedOrder = null" class="bg-white rounded-[2.5rem] w-full max-w-4xl shadow-2xl relative flex flex-col max-h-[90vh]">
            <div class="p-8 pb-4 flex items-center justify-between border-b border-slate-50">
                <div class="flex items-center gap-4">
                    <div class="h-12 w-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 text-xl font-black">
                        <i class="fas fa-receipt"></i>
                    </div>
                    <div>
                        <h3 class="text-xl font-bold text-slate-800" x-text="'Paket Detayı: #' + (selectedOrder?.orderNumber || '')"></h3>
                        <div class="flex items-center gap-3 mt-1">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">N11 PAZARYERİ</span>
                            <span class="w-1 h-1 rounded-full bg-slate-200"></span>
                            <span :class="getStatus(selectedOrder?.shipmentPackageStatus).color" class="px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest" x-text="getStatus(selectedOrder?.shipmentPackageStatus).label"></span>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-8 pt-6 space-y-8">
                    ...
                    <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-xs font-bold text-slate-800" x-text="selectedOrder?.customerfullName"></p>
                            </div>
                            <div>
                                <p class="text-[10px] font-bold text-slate-400">TESLİMAT ADRESİ</p>
                                <p class="text-xs font-bold text-slate-800 leading-relaxed" x-text="selectedOrder?.shippingAddress?.address"></p>
                                <p class="text-[10px] font-black text-brand-500 mt-1 uppercase" x-text="selectedOrder?.shippingAddress?.district + ' / ' + selectedOrder?.shippingAddress?.city"></p>
                            </div>
                        </div>
                    </div>
                    <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs font-black text-emerald-600 tracking-widest" x-text="selectedOrder?.cargoTrackingNumber || 'Henüz Atanmadı'"></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50">
                            <tr class="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                                <th class="px-6 py-3">Ürün Adı / Stok Kodu</th>
                                <th class="px-6 py-3 text-center">Adet</th>
                                <th class="px-6 py-3 text-right">Birim Fiyat</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            <template x-for="item in selectedOrder?.lines" :key="item.productId">
                                <tr>
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-bold text-slate-800" x-text="item.productName"></span>
                                            <span class="text-[10px] text-slate-400 font-bold" x-text="'SKU: ' + (item.stockCode || '-')"></span>
                                        </div>
                                    </td>
                ...
                <div class="space-y-2">
                    ...
                    <pre class="p-6 bg-slate-900 text-emerald-400 rounded-3xl text-[11px] font-mono overflow-x-auto custom-scrollbar shadow-inner" x-text="JSON.stringify(selectedOrder, null, 4)"></pre>
                </div>
            </div>
        </div>
    </div>

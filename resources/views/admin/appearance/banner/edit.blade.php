    ...
    <div class="bg-white rounded-[40px] border border-slate-100 shadow-2xl overflow-hidden p-10">
        <form action="{{ route('admin.appearance.banner.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-8" x-data="{ preview: '{{ asset('storage/' . $banner->image_path) }}' }">
            ...
            <div class="space-y-4" x-data="{ 
                buttons: {{ $banner->buttons ? json_encode($banner->buttons) : '[]' }},
                addButton() {
                ...
                <div class="flex items-center justify-between pl-2">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 font-medium mt-0.5">Banner üzerinde tıklanabilir buton istemiyorsanız tüm butonları silebilirsiniz.</p>
                    </div>
                ...
                <div class="space-y-4">
                    <div x-show="buttons.length === 0" class="p-6 bg-slate-50 border border-slate-200/80 rounded-2xl text-center space-y-2">
                        ...
                        <p class="text-xs font-semibold text-slate-500">Bu banner için buton bulunmuyor.</p>
            ...
            <div class="bg-slate-50 p-8 rounded-[30px] border border-slate-100 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="title_size" value="{{ $banner->title_size ?? 60 }}" min="10" max="150" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-4 focus:ring-emerald-50 outline-none transition-all">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="subtitle_size" value="{{ $banner->subtitle_size ?? 12 }}" min="8" max="50" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-4 focus:ring-emerald-50 outline-none transition-all">
                    </div>

                    <!-- Button BG Color -->
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Buton Arka Plan Rengi</label>
                        <input type="color" name="button_color" value="{{ $banner->button_color ?? '#059669' }}" class="w-full h-12 rounded-xl border border-slate-200 p-1 cursor-pointer">
                    </div>

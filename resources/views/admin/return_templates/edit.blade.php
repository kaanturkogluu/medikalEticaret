...
<div class="max-w-4xl mx-auto space-y-8 pb-20">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight italic uppercase">Şablonu Düzenle</h1>
            <p class="text-sm text-slate-500 mt-1">"{{ $template->name }}" iade kurallarını güncelleyin.</p>
        </div>
    </div>

    <form action="{{ route('admin.return-templates.update', $template) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="bg-white rounded-[40px] border border-slate-100 shadow-xl overflow-hidden p-10 space-y-10">
            <div class="space-y-4">
                ...
                <input type="text" name="name" value="{{ $template->name }}" placeholder="Örn: Mobilya / Sandalye Grubu" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-8 py-5 text-sm font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-brand-50 focus:border-brand-500 transition-all outline-none" required>
            </div>
            ...
            <div class="space-y-4">
                ...
                <textarea name="content" rows="12" class="w-full bg-slate-50 border border-slate-200 rounded-[32px] px-8 py-8 text-sm font-medium text-slate-600 focus:bg-white focus:ring-4 focus:ring-brand-50 focus:border-brand-500 transition-all outline-none leading-relaxed" required>{{ $template->content }}</textarea>
            </div>

...
@section('styles')
<style>
    .btn-emerald { background-color: #059669; transition: all 0.2s; }
    ...
    <div class="w-full max-w-[450px] bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8 md:p-12">
        <div class="text-center mb-10">
            <div class="inline-flex items-center justify-center w-16 h-16 bg-emerald-50 rounded-2xl mb-4">
                <i class="fas fa-user-lock text-emerald-600 text-2xl"></i>
            </div>
            <h1 class="text-3xl font-black tracking-tight text-slate-900 italic uppercase tracking-tighter">Giriş Yap</h1>
        ...
        <form action="{{ route('login.authenticate') }}" method="POST" class="space-y-6" onsubmit="this.submitBtn.disabled = true; this.submitBtn.innerHTML = '<i class=\'fas fa-circle-notch fa-spin mr-2\'></i> Giriş Yapılıyor...';">
            ...
            <div class="flex items-center justify-between px-1">
                <label class="flex items-center cursor-pointer group">
                    <input type="checkbox" name="remember" class="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 transition-all">
                    <span class="ml-2 text-[11px] font-bold text-slate-500 group-hover:text-slate-700 transition-colors">Beni Hatırla</span>
                </label>
                <a href="{{ route('password.request') }}" class="text-[11px] font-black text-emerald-600 hover:text-emerald-700 transition-colors uppercase italic tracking-tighter">Şifremi Unuttum</a>
            </div>

            <button type="submit" name="submitBtn" class="w-full btn-emerald py-4 rounded-2xl text-white font-black text-sm uppercase italic tracking-widest shadow-lg shadow-emerald-600/20 active:scale-[0.98] transition-transform disabled:opacity-70 disabled:cursor-not-allowed">
                Giriş Yap
            </button>
        </form>

        <div class="mt-10 text-center">
            <p class="text-xs text-slate-500 font-medium">Hesabınız yok mu? <a href="{{ route('register') }}" class="font-black text-emerald-600 hover:text-emerald-700 transition-colors uppercase italic tracking-tighter">Hemen Üye Olun</a></p>
        </div>
    </div>
</main>
@endsection

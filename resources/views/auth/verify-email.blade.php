...
<body class="flex flex-col min-h-screen">
    ...
    <main class="flex-grow flex items-center justify-center p-6 py-12">
        <div class="w-full max-w-[440px]">
            ...
            <!-- Alert Messages -->
            @if(session('success'))
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-sm font-semibold text-emerald-700 flex items-center gap-3">
            ...
            <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
                ...
                <!-- Timer -->
                <div class="flex items-center justify-center gap-2 bg-emerald-50 rounded-2xl p-4 mb-6">
                    ...
                    <span class="text-sm font-bold text-emerald-700">Kodun geçerlilik süresi: <span id="countdown" class="font-black">30:00</span></span>
                </div>
                ...
                <form action="{{ route('verify.submit') }}" method="POST" class="space-y-6" onsubmit="this.submitBtn.disabled = true; this.submitBtn.innerHTML = '<i class=\'fas fa-circle-notch fa-spin mr-2\'></i> Doğrulanıyor...';">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="code-input w-full border-2 border-slate-100 rounded-2xl px-4 py-5 text-slate-900 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 bg-slate-50/50 transition-all"
                            placeholder="• • • • • •"
                        >
                    </div>

                    <button type="submit" name="submitBtn" class="btn-primary w-full py-4 rounded-xl text-white font-black text-sm uppercase tracking-widest shadow-lg shadow-emerald-600/20 disabled:opacity-70">
                        Hesabımı Doğrula
                    </button>
                </form>
                ...
                <div class="mt-6 pt-6 border-t border-slate-100 text-center">
                    ...
                    <form action="{{ route('verify.resend') }}" method="POST" onsubmit="localStorage.setItem('resend_cooldown_end', Date.now() + 60000); this.resendBtn.disabled = true; this.resendBtn.innerHTML = '<i class=\'fas fa-circle-notch fa-spin mr-2\'></i> Gönderiliyor...';">
                        @csrf
                        <input type="hidden" name="email" value="{{ $email }}">
                        <button type="submit" name="resendBtn" id="resendBtn" class="text-sm font-bold text-emerald-600 hover:underline inline-flex items-center gap-2 disabled:opacity-70 disabled:grayscale disabled:no-underline">
            ...
            <p class="text-center text-xs text-slate-400 mt-6">
                Yanlış e-posta mı?
                <a href="{{ route('register') }}" class="font-bold text-emerald-600 hover:underline">Tekrar kayıt olun</a>
            </p>
        </div>
    </main>

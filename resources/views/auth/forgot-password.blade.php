...
<main class="flex-grow flex items-center justify-center p-6 py-20 bg-gray-50/50">
    <div class="w-full max-w-[450px] bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8 md:p-12">
        <div class="text-center mb-10">
            ...
            <h1 class="text-3xl font-black tracking-tight text-slate-900 italic uppercase tracking-tighter">Şifremi Unuttum</h1>
            <p class="text-slate-500 mt-2 text-sm font-medium">E-posta adresinizi girerek şifre sıfırlama bağlantısı isteyin.</p>
        </div>

        @if(session('success'))
        <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 rounded-2xl text-[11px] font-black uppercase tracking-widest text-emerald-600 flex items-center gap-3">
        ...
        <form action="{{ route('password.email') }}" method="POST" class="space-y-6" onsubmit="this.submitBtn.disabled = true; this.submitBtn.innerHTML = '<i class=\'fas fa-circle-notch fa-spin mr-2\'></i> Gönderiliyor...';">
            ...
            <button type="submit" name="submitBtn" class="w-full bg-emerald-600 hover:bg-emerald-700 py-4 rounded-2xl text-white font-black text-sm uppercase italic tracking-widest shadow-lg shadow-emerald-600/20 active:scale-[0.98] transition-transform disabled:opacity-70 disabled:cursor-not-allowed">
                Bağlantı Gönder
            </button>
        </form>

        <div class="mt-10 text-center">
            <p class="text-xs text-slate-500 font-medium">Şifrenizi hatırladınız mı? <a href="{{ route('login') }}" class="font-black text-emerald-600 hover:text-emerald-700 transition-colors uppercase italic tracking-tighter">Giriş Yapın</a></p>
        </div>
    </div>
</main>
@endsection
